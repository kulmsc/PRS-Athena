...
for q in allFilesReal:
	print(q.split('.')[1].split('-'))
	if q.split('.')[1].split('-')[1]==method: #change to either 3 or 1
		allFiles.append(q)
if len(allFiles)==0:
	print("no files")
	sys.exit()
...
for data in allFiles:
	splitData=data.split('.')[1].split('-')[0]
	print("splitData")
	print(splitData)
	for i in range(numberSubFiles):
		if splitData==str(i+1):  #THIS IS WHERE I CHANGE to either 4 or 5
